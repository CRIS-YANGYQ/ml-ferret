...
def find_target_bboxes(input_str, target_str=None, is_query_all=0):
    """
    ...
    if is_query_all == 0:
        ...
        if target_str is not None:
            target_str = target_str.lower()
            matches = re.findall(fr'{re.escape(target_str)}\s*{bbox_pattern}', input_str)
        else:
            matches = re.findall(bbox_pattern, input_str)
        sliced_str = input_str
        result_lst = []
        for match in matches:
            # 格式化当前bbox为字符串形式
            bbox_str = f"[{', '.join(match)}]"
            result_lst.append(eval(bbox_str))
        return result_lst
...
if __name__ == "__main__":
    ...
    parser.add_argument("--result_path", type=str, default="/comp_robot/yangyuqin/workplace/Multi-model/result/coco_obj_detection/ferret-7b.json")
    ...
    for element_idx, element in tqdm(enumerate(dataset)):
        ...
        if args.is_query_all == 1:
            ...
            for i, category in enumerate(category_names):
                bbox_lst = find_target_bboxes(outputs, category, is_query_all=1)
